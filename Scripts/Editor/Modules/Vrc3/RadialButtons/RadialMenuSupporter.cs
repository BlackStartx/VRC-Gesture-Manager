...
{
    public class RadialMenuSupporter : RadialMenuItem
    {
        private readonly Func<float, float> ease = Easing.Linear;
        ...
        private ValueAnimation<float> _idle;

        private RadialMenuSupporter(Vrc3Supporter.Supporter supporter, bool early, int index, int pool, int offset, Texture2D icon) : base(supporter.name, icon, null)
        {
            _pool = pool;
            _index = index;
            _early = early;
        ...
        public RadialMenuSupporter(bool early, int index, int pool, int offset, Texture2D icon) : this(Vrc3Supporter.SupporterData(early, index), early, index, pool, offset, icon)
        {
        }
        ...
        private void Fade(VisualElement element, float value)
        {
            ...
            if (!_early && Selected) CircleElement.CenterColor = Color.Lerp(RadialMenuUtility.Colors.RadialSelColor, _supporter.background, value);
        }

        private void StartOffset(Action<VisualElement, float> change = null) => DataHolder.experimental.animation.Start(From, To, _offset, change).Ease(ease).OnCompleted(StartFadeOut);

        private void StartFadeOut()
        {
            _shouldFade = Vrc3Supporter.ShouldFade(_early, _pool);
            DataHolder.experimental.animation.Start(From, 0f, Duration, FadeOut).Ease(ease).OnCompleted(StartFadeIn);
        }

        private void StartFadeIn()
        {
            SetSupporter(Vrc3Supporter.SupporterData(_early, _index += _pool));
            DataHolder.experimental.animation.Start(0f, To, Duration, FadeIn).Ease(ease).OnCompleted(StartFadeOut);
        }
