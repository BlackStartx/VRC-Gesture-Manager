...
namespace GestureManager.Scripts.Core.VisualElements
{
    public class GmgTmpRichTextElement : VisualElement
    {
        ...
        private class Data
        {
            ...
            public StyleValue<Color> ColorStyle(StyleValue<Color> fallback) => TextColor.LastOrDefault() ?? fallback;
            public StyleValue<int> SizeStyle(StyleValue<int> fallback) => Size.LastOrDefault() ?? fallback;
            public StyleValue<Color> MarkStyle => Mark.LastOrDefault() ?? Color.clear;

            internal int Italic;
            internal int Bold;
            internal readonly List<Color?> Mark = new List<Color?>();
            internal readonly List<Color?> TextColor = new List<Color?>();
            internal readonly List<StyleValue<int>?> Size = new List<StyleValue<int>?>();
            ...
            public static bool HandleStyleLengthList(IList<StyleValue<int>?> list, bool close, string attribute) => HandleList(list, close, attribute, TryAddStyleLength);

            private static bool TryAddStyleLength(ICollection<StyleValue<int>?> list, string attribute)
            {
                if (attribute == null || !int.TryParse(attribute, out var intSize)) return false;
                var item = new StyleValue<int>(intSize);
                list.Add(item);
                return true;
            }
        ...
        public GmgTmpRichTextElement()
        {
            ...
            Add(_textHolder = new VisualElement { pickingMode = PickingMode.Ignore, style = { positionType = PositionType.Absolute } });
        ...
        private void AddToken(string tokenInput)
        {
            ...
            {
                style = { fontStyleAndWeight = _data.FontStyle, color = _data.ColorStyle(style.color), backgroundColor = _data.MarkStyle, fontSize = _data.SizeStyle(style.fontSize) },
